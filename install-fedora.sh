...
install_dependencies() {
  if is_arch; then
    ...
    run_command "sudo dnf install -y build-essential curl file git unzip fontconfig"
    run_command "curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh"
    run_command ". $HOME/.cargo/env"
  fi
}
  ...
  "alacritty")
    if ! command -v alacritty &>/dev/null; then
      if is_arch; then
        ...
        install_terminal_with_progress "Alacritty" "sudo dnf install alacritty" "mkdir -p ~/.config/alacritty && cp alacritty.toml ~/.config/alacritty/alacritty.toml"
      fi
    else
  ...
  "kitty")
    if [ "$os_choice" = "mac" ]; then
      if ! command -v kitty &>/dev/null; then
        install_terminal_with_progress "Kitty" "brew install --cask kitty" "mkdir -p ~/.config/kitty"
      else
  ...
  "ghostty")
    if ! command -v ghostty &>/dev/null; then
      if is_arch; then
        ...
        install_terminal_with_progress "Ghostty" "brew install --cask ghostty" 
      fi
    else
...
if [ "$os_choice" = "linux" ]; then
  if ! is_arch; then
    ...
    install_dependencies_with_progress "sudo dnf update && sudo dnf upgrade -y"
  fi
fi
